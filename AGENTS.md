...
Read `sdkwork.app.config.json` only when the task touches Dezhou application behavior, runtime config, SDK wiring, release metadata, app-owned capabilities, packaging, or deployment.
...
- `sdkwork.app.config.json`: Dezhou application identity, runtime, release, and capability metadata.
...
- `apis/`: Dezhou-owned API contract sources and materialized OpenAPI inputs.
- `apps/`: runnable Dezhou application roots and application surfaces.
- `crates/`: reusable Rust crates and router crates.
...
- `pnpm dev`: default local development for the PC browser surface.
...
- `cargo test --workspace`, `cargo fmt --all --check`: Rust verification.
