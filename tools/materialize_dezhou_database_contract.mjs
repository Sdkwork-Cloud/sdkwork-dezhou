...
const baselinePath = path.join(root, 'database/ddl/baseline/postgres/0001_dezhou_baseline.sql');
...
const prefixRegistry = {
  ...
  prefixes: [{ prefix: 'dezhou_', owner: 'dezhou-platform', domain: 'game' }],
};
...
const schemaYaml = [
  ...
  'owner_team: dezhou-platform',
  'compliance_level: L2',
  'engines:',
  '  - postgres',
  'table_prefix: dezhou_',
  'tables:',
  ...tableNames.map(
    (name) => `  - name: ${name}\n    lifecycle_status: active\n    owner: dezhou-platform`,
  ),
...
process.stdout.write(`materialized ${tableNames.length} tables into dezhou database contract\n`);
