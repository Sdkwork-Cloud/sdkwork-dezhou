...
const FAMILY_DEFAULTS = {
  'sdkwork-dezhou-app-sdk': { surface: 'app', clientName: 'SdkworkDezhouAppClient' },
  'sdkwork-dezhou-backend-sdk': { surface: 'backend', clientName: 'SdkworkDezhouBackendClient' },
};
...
function createGenerationPlans(rootDir, options) {
  ...
  assert.equal(assembly.sdkOwner, 'sdkwork-dezhou');
  ...
  for (const language of requestedLanguages) {
    if (!SUPPORTED_LANGUAGES.has(language)) {
      throw new Error(`Unsupported Dezhou standard SDK language: ${language}.`);
    }
  }
  ...
  const defaults = FAMILY_DEFAULTS[options.sdkFamily] ?? FAMILY_DEFAULTS['sdkwork-dezhou-app-sdk'];
...
export function generateDezhouSdk(options = {}, rootDir = process.cwd()) {
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  try {
    const result = generateDezhouSdk(parseArgs(process.argv.slice(2)));
